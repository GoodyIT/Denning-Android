...
//        dashboardList.setLayoutManager(new StickyHeaderLayoutManager());
    ...
    void fetchHeader() {
        new Thread(new Runnable() {
            public void run() {
                String url  = DISharedPreferences.getInstance(getApplicationContext()).getServerAPI() + DIConstants.DASHBOARD_S1_MATTERLISTING_GET_URL;
                ...
                try {
                    ...
                    if (response != null) {
                        if (!response.isSuccessful()) {
                            ...
                            try {
                                fileList = ThreeItemModel.getThreeItemFromResponse(new JSONObject(response.body().string()));
