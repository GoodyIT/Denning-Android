...
public class LegalFirmAdapter extends SectioningAdapter {
    LegalFirm mLegalFirm;
    Context mContext;
    String TO, SUBJECT, MESSAGE ;
    OnItemClickListener clickListener;

    LegalFirmAdapter(LegalFirm legalFirm, Context context) {
        this.mLegalFirm = legalFirm;
    ...
    public int getNumberOfItemsInSection(int sectionIndex) {
        ...
        if (sectionIndex == 0) {
            ...
            if (mLegalFirm.relatedMatter != null) {
                count =  mLegalFirm.relatedMatter.size();
            } else {
                count = 0;
            }
        }
    ...
    public SectioningAdapter.ItemViewHolder onCreateItemViewHolder(ViewGroup parent, int itemUserType) {
        switch (itemUserType) {
            case DIConstants.HEADER_TYPE:
                ...
                return new LegalFirmAdapter.HeaderTypeViewHolder(itemView);
            case DIConstants.GENERAL_TYPE:
                itemView = LayoutInflater.
                        from(parent.getContext()).
                        inflate(R.layout.cardview_search_general, parent, false);

                return new LegalFirmAdapter.GeneralTypeViewHolder(itemView);
            case DIConstants.LAST_TYPE:
                itemView = LayoutInflater.
                        from(parent.getContext()).
                        inflate(R.layout.cardview_search_last, parent, false);

                return new LegalFirmAdapter.LastTypeViewHolder(itemView);
            default:
                break;
        }
    ...
    private void displayHeader(RecyclerView.ViewHolder holder, int position) {
        ...
        headerTypeViewHolder.headerName.setText(mLegalFirm.name);
        headerTypeViewHolder.headerIDNo.setText(mLegalFirm.IDNo);
    }
    ...
    private void displayGeneral(RecyclerView.ViewHolder holder, int position) {
        ...
        switch (position) {
            case 0:
                setupGeneralTypeHoler(generalTypeViewHolder, "Phone 1", mLegalFirm.homePhone, View.VISIBLE, R.drawable.ic_phone_red);
                break;
            case 1:
                setupGeneralTypeHoler(generalTypeViewHolder, "Phone 2", mLegalFirm.homePhone, View.VISIBLE, R.drawable.ic_phone_red);
                break;
            case 2:
                setupGeneralTypeHoler(generalTypeViewHolder, "Phone (mobile)", mLegalFirm.mobilePhone, View.VISIBLE, R.drawable.ic_phone_red);
                break;
            case 3:
                setupGeneralTypeHoler(generalTypeViewHolder, "Phone (office)", mLegalFirm.officePhone, View.VISIBLE, R.drawable.ic_phone_red);
                break;
            case 4:
                setupGeneralTypeHoler(generalTypeViewHolder, "Email", mLegalFirm.email, View.VISIBLE, R.drawable.ic_email_red);
                break;
            case 5:
                setupGeneralTypeHoler(generalTypeViewHolder, "Address", mLegalFirm.address.fullAddress, View.VISIBLE, R.drawable.ic_location_red);
                break;
    ...
    private void displayLast(RecyclerView.ViewHolder holder, int position) {
        ...
        String[] fileNoName = DIHelper.removeFileNoFromMatterTitle(mLegalFirm.relatedMatter.get(position).title);
        lastTypeViewHolder.fileNo.setText(fileNoName[0]);
        lastTypeViewHolder.name.setText(fileNoName[1]);
        lastTypeViewHolder.date.setText(DIHelper.convertToSimpleDateFormat(mLegalFirm.relatedMatter.get(position).sortDate));
    }
    @Override
