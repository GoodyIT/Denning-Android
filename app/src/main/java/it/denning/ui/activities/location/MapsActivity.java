package it.denning.ui.activities.location;
...
public class MapsActivity extends BaseLoggableActivity
    ...
    void _gotoGoogleMap(JsonObject jsonElement) {
        String addr = jsonElement.get("results").getAsJsonArray().get(0).getAsJsonObject().get("formatted_address").getAsString();
        String dest = android.text.TextUtils.join(",", addr.split(" "));
    ...
    void gotoGoogleMap()  {
        ...
        NetworkManager.getInstance().sendGoogleGet(latlng, new CompositeCompletion() {
            @Override
            public void parseResponse(JsonElement jsonElement) {
                _gotoGoogleMap(jsonElement.getAsJsonObject());
            }
        });
    }
