package it.denning.ui.fragments.mediapicker;
...
public class MediaPickHelperFragment extends Fragment {
        ...
        if (isResultFromMediaPick(requestCode, resultCode, data)) {
            if (requestCode == MediaUtils.IMAGE_VIDEO_LOCATION_REQUEST_CODE) {
                ...
                if ((requestCode == MediaUtils.CAMERA_PHOTO_REQUEST_CODE || requestCode == MediaUtils.CAMERA_VIDEO_REQUEST_CODE) && (data == null || data.getData() == null)) {
                    ...
                    data = new Intent();
                    data.setData(MediaUtils.getValidUri(MediaUtils.getLastUsedCameraFile(), this.getActivity()));
                }
    ...
    private boolean
    isResultFromMediaPick(int requestCode, int resultCode, Intent data) {
        return resultCode == Activity.RESULT_OK && ((requestCode == MediaUtils.CAMERA_PHOTO_REQUEST_CODE || requestCode == MediaUtils.DENNING_FILE_REQUEST_CODE || requestCode == MediaUtils.CAMERA_VIDEO_REQUEST_CODE) || (requestCode == MediaUtils.GALLERY_REQUEST_CODE && data != null)
